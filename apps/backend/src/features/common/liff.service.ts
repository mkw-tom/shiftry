...
export async function verifyIdToken(idToken: string) {
  const { alg } = decodeProtectedHeader(idToken);
	...
	if (process.env.TEST_MODE === "true") {
		const secret = new TextEncoder().encode(process.env.TEST_IDTOKEN_HS256_SECRET );
  ...
  throw new Error(`Unsupported alg: ${alg}. Use real LIFF idToken (RS256) or enable HS256 test.`);
}
