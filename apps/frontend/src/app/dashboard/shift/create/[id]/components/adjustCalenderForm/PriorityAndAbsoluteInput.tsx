...
const PriorityAndAbsoluteInput = ({ control }: Props) => {
	...
	const handlePickMember = (user: UserLite) => {
		if (showMemberList.mode === "absolute") {
			if (priority.find((u: PriorityUserType) => u.id === user.id)) return;
			...
			if (absolute.find((u: AbsoluteUserType) => u.id === user.id)) return;
	...
	const handleRemoveAbsolute = (id: string) => {
		const idx = absolute.findIndex((x: AbsoluteUserType) => x.id === id);
		if (idx >= 0) removeAbs(idx);
	};

	const handleRemovePriority = (id: string) => {
		const idx = priority.findIndex((x: PriorityUserType) => x.id === id);
