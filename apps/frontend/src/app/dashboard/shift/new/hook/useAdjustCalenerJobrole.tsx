...
const useAdjustCalenerJobrole = ({
	editCalendarPosition,
	setEditCalendarPosition,
	jobRoles,
	setJobRoles,
}: {
	editCalendarPosition: TimeSlotType;
	...
	const addJobRoleFromAllJobRolesList = (jobRoleValue: string) => {
		if (editCalendarPosition.jobRoles.includes(jobRoleValue)) {
	...
	const addJobRole = (inputJobRoleValue: string) => {
		if (editCalendarPosition.jobRoles.includes(inputJobRoleValue)) {
			window.alert("この業務はすでに追加されています");
			return;
		}
...
export default useAdjustCalenerJobrole;
