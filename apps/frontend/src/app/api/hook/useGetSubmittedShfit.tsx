...
export const useGetSubmittedShfit = () => {
	const [isLoading, setIsLoading] = useState(false);
	const [error, setError] = useState<string | null>(null);
	const dispatch = useDispatch<AppDispatch>();
	const { jwt } = useSelector((state: RootState) => state.authToken);

	const getSubmittedShfit = useCallback(
		async ({
			shiftRequestId,
		}: {
			shiftRequestId: string;
		}): Promise<GetSubmittedShiftsSpecificResponse | ErrorResponse> => {
			setIsLoading(true);
			setError(null);
			try {
				if (!jwt) {
				...
				const res = await useFetch<GetSubmittedShiftsSpecificResponse>({
					jwt,
					method: "GET",
					path: submittedShiftApi.index(shiftRequestId),
				});
	...
	return { getSubmittedShfit, isLoading, error };
};
