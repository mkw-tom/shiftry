					...
					<tbody className="">
						{sortedMembers.map((member) => (
							<tr key={member.user.id}>
								<td
									className={`font-bold flex flex-col items-center gap-1 py-2 sticky left-0 border border-gray01 ${user?.id === member.user.id ? "bg-green03" : "bg-white "}`}
								>
									<img
								...
								{weekDates.map((date) => {
									...
									return (
										<td
											key={dateKey}
											className={`text-xs w-20 text-center align-middle border border-gray01 ${user?.id === member.user.id ? "bg-green03" : "bg-white "}`}
										>
											{assigned.length > 0 ? (
												<ul>
