...
import { ShiftsOfAssignType } from "@shared/common/types/json";
...
const ShiftContent = () => {
    const { id } = useParams();
    const { userToken, storeToken } = useSelector((state: RootState) => state.token);
    const [assignShift, setAssignShift] = useState<AssignShiftWithJson>(dummyAssignShift);
    ...
    function getDatesArray(weekStart: Date, weekEnd: Date): { label: string; key: string }[] {
      ...
      const dates = getDatesArray(shiftRequest.weekStart as Date, shiftRequest.weekEnd as Date);
    ...
    return (
        <main className="bg-base w-full h-lvh">
            <div className="w-full h-full pt-4 flex flex-col items-center gap-5">
                <Head dates={dates} tableSlice={tableSlice} setTableSlice={setTableSlice} />
                <ShiftTable dates={slicedDates} assignShift={assignShift} shiftRequest={shiftRequest} />
            </div>
